...
fn uninvited_judgement_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Identity::identity(ten).unwrap().0.judgements,
...
fn set_username_with_acceptance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            AccountOfUsername::<Test>::get::<&Username<Test>>(&full_username),
            Some(who)
        );
    });
}
...
fn removing_dangling_usernames_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Identity::clear_identity(RuntimeOrigin::signed(
            who_account.clone()
        ),));
